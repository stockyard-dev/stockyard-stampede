...
async function load(){var r=await fetch(A+'/load_tests').then(function(r){return r.json()});items=r.load_tests||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.target_url||'').toLowerCase().includes(q)||(i.concurrency||'').toLowerCase().includes(q)||(i.duration_seconds||'').toLowerCase().includes(q)});
...
if(i.target_url)h+='<div class="item-sub">'+esc(i.target_url)+'</div>';
...
if(i.concurrency)h+='<span>'+esc(i.concurrency)+'</span>';
if(i.duration_seconds)h+='<span>'+esc(i.duration_seconds)+'</span>';
if(i.request_count)h+='<span>'+esc(i.request_count)+'</span>';
if(i.avg_latency_ms)h+='<span>'+esc(i.avg_latency_ms)+'</span>';
if(i.error_rate)h+='<span>'+esc(i.error_rate)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/load_tests/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",target_url:"",concurrency:"",duration_seconds:"",request_count:"",avg_latency_ms:"",error_rate:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' STAMPEDE</h2>';
...
h+='<div class="fr"><label>Concurrency</label><input id="f-concurrency" value="'+esc(i.concurrency)+'"></div>';
h+='<div class="fr"><label>Duration</label><input id="f-duration-seconds" value="'+esc(i.duration_seconds)+'"></div>';
h+='<div class="fr"><label>Request Count</label><input id="f-request-count" value="'+esc(i.request_count)+'"></div>';
h+='<div class="fr"><label>Avg Latency</label><input id="f-avg-latency-ms" value="'+esc(i.avg_latency_ms)+'"></div>';
h+='<div class="fr"><label>Error Rate</label><input id="f-error-rate" value="'+esc(i.error_rate)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),target_url:document.getElementById('f-target-url').value.trim(),concurrency:document.getElementById('f-concurrency').value.trim(),duration_seconds:document.getElementById('f-duration-seconds').value.trim(),request_count:document.getElementById('f-request-count').value.trim(),avg_latency_ms:document.getElementById('f-avg-latency-ms').value.trim(),error_rate:document.getElementById('f-error-rate').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/load_tests/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/load_tests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
